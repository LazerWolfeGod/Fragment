...
class Environment_Data:
    ...
    def process_tile_overlays():
        ...
        for t in Environment_Data.tile:
            image = pygame.image.load(Environment_Data.tile[t]['File']).convert_alpha()
            rec = Environment_Data.tile[t]['Rect']
            image_grid = [[]]
            x = 0
            y = 0
            while True:
                ...
                if rec[0]+x*(rec[0]+rec[2])>=image.get_width():
                    ...
                    image_grid.append([])
                    x = 0
            Environment_Data.tile[t]['Tile']['base_image'] = image_grid[Environment_Data.tile[t]['Tile']['atlas'][1]][Environment_Data.tile[t]['Tile']['atlas'][0]]
            new_overlays = {}
            for o in Environment_Data.tile[t]['overlays']:
                if Environment_Data.tile[t]['overlays'][o]['atlas']!=-1:
                    pos = Environment_Data.tile[t]['overlays'][o]['atlas']
                    hitbox = Environment_Data.tile[t]['overlays'][o]['hitbox'][:]
                    st = o
                    image = image_grid[pos[1]][pos[0]]
                    Environment_Data.tile[t]['overlays'][o]['base_image'] = image
                    for r in range(pos[2]):
                        ...
                        new_overlays[st] = {'base_image':image,
                                            'hitbox':hitbox,
                                            'atlas':-1}
            Environment_Data.tile[t]['overlays'].update(new_overlays)

            for o in Environment_Data.tile[t]['overlays']:
                Environment_Data.tile[t]['overlays'][o]['Layer'] = Environment_Data.tile[t]['Layer']
                    
    
    def resize_tiles(scale,pixels=False):
        Environment_Data.process_tile_overlays()
        for t in Environment_Data.tile:
            img = Environment_Data.tile[t]['Tile']['base_image']
            if not pixels: rec = (img.get_width()*scale,img.get_height()*scale)
            else: rec = (scale,scale)
            Environment_Data.tile[t]['Tile']['image'] = pygame.transform.scale(img,rec)
            for i in Environment_Data.tile[t]['overlays']:
                Environment_Data.tile[t]['overlays'][i]['image'] = pygame.transform.scale(Environment_Data.tile[t]['overlays'][i]['base_image'],rec)


Environment_Data.resize_tiles(1)
